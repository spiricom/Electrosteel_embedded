...
static uint8  USB_1_started = 0u;


/*******************************************************************************
* Function Name: USB_1_CyBtldrCommStart
****************************************************************************//**
*
...
void USB_1_CyBtldrCommStart(void) 
{
    ...
    USB_1_Start(0u, USB_1_DWR_POWER_OPERATION);
...
/*******************************************************************************
* Function Name: USB_1_CyBtldrCommStop.
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USB_1_CyBtldrCommReset.
****************************************************************************//**
*
...
{
    USB_1_EnableOutEP(USB_1_BTLDR_OUT_EP); 
}


/*******************************************************************************
* Function Name: USB_1_CyBtldrCommWrite.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    USB_1_LoadInEP(USB_1_BTLDR_IN_EP, pData, USB_1_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait unitl host reads data from IN endpoint. */
    while ((USB_1_GetEPState(USB_1_BTLDR_IN_EP) == USB_1_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(USB_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (USB_1_GetEPState(USB_1_BTLDR_IN_EP) == USB_1_IN_BUFFER_FULL)
    {
...
/*******************************************************************************
* Function Name: USB_1_CyBtldrCommRead.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    if (size > USB_1_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = USB_1_BTLDR_SIZEOF_WRITE_BUFFER;
    }
    ...
    if (0u != USB_1_started)
    {
        ...
        while ((0u == USB_1_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(USB_1_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        if (0u != USB_1_GetConfiguration())
        {
            (void) USB_1_IsConfigurationChanged();  /* Clear configuration changes state status. */
            USB_1_CyBtldrCommReset();
    ...
    {
        if (0u != USB_1_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET. */
        {
            if (0u != USB_1_GetConfiguration())   /* Init OUT endpoints when device reconfigured. */
            {
                USB_1_CyBtldrCommReset();
            }
        }
    ...
    while ((USB_1_GetEPState(USB_1_BTLDR_OUT_EP) != USB_1_OUT_BUFFER_FULL) && \
           (0u != timeoutMs))
    {
        CyDelay(USB_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (USB_1_GetEPState(USB_1_BTLDR_OUT_EP) == USB_1_OUT_BUFFER_FULL)
    {
        *count = USB_1_ReadOutEP(USB_1_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
