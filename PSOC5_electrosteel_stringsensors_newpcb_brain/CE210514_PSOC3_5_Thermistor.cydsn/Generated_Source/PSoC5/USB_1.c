...
* functions.
*/
uint8 USB_1_initVar = 0u;

#if (USB_1_EP_MANAGEMENT_DMA)
    #if (CY_PSOC4)
        static void USB_1_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 USB_1_DmaChan[USB_1_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    #else
        /* DMA chanels assigend for endpoints. */
        uint8 USB_1_DmaChan[USB_1_MAX_EP];
        
        /* DMA TDs require for PSoC 3/5LP operation. */
        uint8 USB_1_DmaTd[USB_1_MAX_EP];
    #endif /* (CY_PSOC4) */
    ...
    uint8  USB_1_DmaEpBurstCnt   [USB_1_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  USB_1_DmaEpLastBurstEl[USB_1_MAX_EP];

    /* Storage for arrays above. */
    uint8  USB_1_DmaEpBurstCntBackup  [USB_1_MAX_EP];
    uint32 USB_1_DmaEpBufferAddrBackup[USB_1_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 USB_1_DmaReqOut[USB_1_MAX_EP] =
    {
        0u,
        USB_1_ep1_dma__TR_OUTPUT,
        USB_1_ep2_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 USB_1_DmaBurstEndOut[USB_1_MAX_EP] =
    {
        0u,
        USB_1_BURSTEND_0_TR_OUTPUT,
        USB_1_BURSTEND_1_TR_OUTPUT,
        USB_1_BURSTEND_2_TR_OUTPUT,
        USB_1_BURSTEND_3_TR_OUTPUT,
        USB_1_BURSTEND_4_TR_OUTPUT,
        USB_1_BURSTEND_5_TR_OUTPUT,
        USB_1_BURSTEND_6_TR_OUTPUT,
        USB_1_BURSTEND_7_TR_OUTPUT
    };
    
#else
    #if (USB_1_EP_DMA_AUTO_OPT == 0u)
        static uint8 clearInDataRdyStatus = USB_1_ARB_EPX_CFG_DEFAULT;
        uint8 USB_1_DmaNextTd[USB_1_MAX_EP];
        const uint8 USB_1_epX_TD_TERMOUT_EN[USB_1_MAX_EP] =
        {
            0u,
            0u,
            0u,
        ...
        volatile uint16 USB_1_inLength[USB_1_MAX_EP];
        const uint8 *USB_1_inDataPointer[USB_1_MAX_EP];
        volatile uint8 USB_1_inBufFull[USB_1_MAX_EP];
...
*   following list.
*
*       *USB_1_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *USB_1_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *USB_1_DWR_POWER_OPERATION* - Enable or disable the voltage
...
void USB_1_Start(uint8 device, uint8 mode) 
{
    ...
    USB_1_InitComponent(device, mode);
}


/*******************************************************************************
* Function Name: USB_1_Init
****************************************************************************//**
*
...
* USB_1_Init() because the USB_1_Start() routine calls
...
void USB_1_Init(void) 
{
    ...
    * USB_1_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    USB_1_USBIO_CR1_REG &= ~ (uint32) USB_1_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    USB_1_BUS_RST_CNT_REG = USB_1_DEFUALT_BUS_RST_CNT;
    ...
    */
    USB_1_POWER_CTRL_REG = USB_1_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    USB_1_POWER_CTRL_REG |= USB_1_DEFAULT_POWER_CTRL_PHY;
    ...
    USB_1_CR1_REG = USB_1_DEFUALT_CR1;
    ...
    USB_1_INTR_SIE_MASK_REG = USB_1_DEFAULT_INTR_SIE_MASK;
    ...
    USB_1_CR1_REG = USB_1_CR1_ENABLE_LOCK;
    ...
    */
    ...
    USB_1_USBIO_CR1_REG &= (uint8) ~USB_1_USBIO_CR1_IOMODE;
    ...
    CyDelayUs(USB_1_WAIT_REG_STABILITY_1US);
    ...
    USB_1_DM_INP_DIS_REG &= (uint8) ~USB_1_DM_MASK;
    USB_1_DP_INP_DIS_REG &= (uint8) ~USB_1_DP_MASK;
    ...
    CY_SET_REG8(USB_1_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(USB_1_ARB_RW1_WA_MSB_PTR, 0u);

#if (USB_1_EP_MANAGEMENT_DMA)
    /* Initialize transfer descriptor. This will be used to detect DMA state - initialized or not. */
    for (i = 0u; i < USB_1_MAX_EP; ++i)
    {
        USB_1_DmaTd[i] = DMA_INVALID_TD;

    #if (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u))
        USB_1_DmaNextTd[i] = DMA_INVALID_TD;
    ...
    CyIntSetPriority     (USB_1_INTR_HI_VECT_NUM,  USB_1_INTR_HI_PRIORITY);
    (void) CyIntSetVector(USB_1_INTR_HI_VECT_NUM, &USB_1_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (USB_1_INTR_LO_VECT_NUM,  USB_1_INTR_LO_PRIORITY);
    (void) CyIntSetVector(USB_1_INTR_LO_VECT_NUM, &USB_1_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (USB_1_INTR_MED_VECT_NUM,  USB_1_INTR_MED_PRIORITY);
    (void) CyIntSetVector(USB_1_INTR_MED_VECT_NUM, &USB_1_INTR_MED_ISR);

#else
    /* Set bus reset interrupt. */
    CyIntSetPriority(USB_1_BUS_RESET_VECT_NUM, USB_1_BUS_RESET_PRIOR);
    (void) CyIntSetVector(USB_1_BUS_RESET_VECT_NUM,   &USB_1_BUS_RESET_ISR);

    /* Set Control Endpoint Interrupt. */
    CyIntSetPriority(USB_1_EP_0_VECT_NUM, USB_1_EP_0_PRIOR);
    (void) CyIntSetVector(USB_1_EP_0_VECT_NUM,   &USB_1_EP_0_ISR);

    /* Set SOF interrupt. */
    #if (USB_1_SOF_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_SOF_VECT_NUM,  USB_1_SOF_PRIOR);
        (void) CyIntSetVector(USB_1_SOF_VECT_NUM, &USB_1_SOF_ISR);
    #endif /* (USB_1_SOF_ISR_ACTIVE) */

    /* Set Data Endpoint 1 Interrupt. */
    #if (USB_1_EP1_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_1_VECT_NUM,  USB_1_EP_1_PRIOR);
        (void) CyIntSetVector(USB_1_EP_1_VECT_NUM,  &USB_1_EP_1_ISR);
    #endif /* (USB_1_EP1_ISR_ACTIVE) */

    /* Set Data Endpoint 2 Interrupt. */
    #if (USB_1_EP2_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_2_VECT_NUM,  USB_1_EP_2_PRIOR);
        (void) CyIntSetVector(USB_1_EP_2_VECT_NUM, &USB_1_EP_2_ISR);
    #endif /* (USB_1_EP2_ISR_ACTIVE) */

    /* Set Data Endpoint 3 Interrupt. */
    #if (USB_1_EP3_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_3_VECT_NUM,  USB_1_EP_3_PRIOR);
        (void) CyIntSetVector(USB_1_EP_3_VECT_NUM, &USB_1_EP_3_ISR);
    #endif /* (USB_1_EP3_ISR_ACTIVE) */

    /* Set Data Endpoint 4 Interrupt. */
    #if (USB_1_EP4_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_4_VECT_NUM,  USB_1_EP_4_PRIOR);
        (void) CyIntSetVector(USB_1_EP_4_VECT_NUM, &USB_1_EP_4_ISR);
    #endif /* (USB_1_EP4_ISR_ACTIVE) */

    /* Set Data Endpoint 5 Interrupt. */
    #if (USB_1_EP5_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_5_VECT_NUM,  USB_1_EP_5_PRIOR);
        (void) CyIntSetVector(USB_1_EP_5_VECT_NUM, &USB_1_EP_5_ISR);
    #endif /* (USB_1_EP5_ISR_ACTIVE) */

    /* Set Data Endpoint 6 Interrupt. */
    #if (USB_1_EP6_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_6_VECT_NUM,  USB_1_EP_6_PRIOR);
        (void) CyIntSetVector(USB_1_EP_6_VECT_NUM, &USB_1_EP_6_ISR);
    #endif /* (USB_1_EP6_ISR_ACTIVE) */

     /* Set Data Endpoint 7 Interrupt. */
    #if (USB_1_EP7_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_7_VECT_NUM,  USB_1_EP_7_PRIOR);
        (void) CyIntSetVector(USB_1_EP_7_VECT_NUM, &USB_1_EP_7_ISR);
    #endif /* (USB_1_EP7_ISR_ACTIVE) */

    /* Set Data Endpoint 8 Interrupt. */
    #if (USB_1_EP8_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_EP_8_VECT_NUM,  USB_1_EP_8_PRIOR);
        (void) CyIntSetVector(USB_1_EP_8_VECT_NUM, &USB_1_EP_8_ISR);
    #endif /* (USB_1_EP8_ISR_ACTIVE) */

    /* Set ARB Interrupt. */
    #if (USB_1_EP_MANAGEMENT_DMA && USB_1_ARB_ISR_ACTIVE)
        CyIntSetPriority     (USB_1_ARB_VECT_NUM,  USB_1_ARB_PRIOR);
        (void) CyIntSetVector(USB_1_ARB_VECT_NUM, &USB_1_ARB_ISR);
    #endif /* (USB_1_EP_MANAGEMENT_DMA && USB_1_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (USB_1_DP_ISR_ACTIVE)
    CyIntSetPriority     (USB_1_DP_INTC_VECT_NUM,  USB_1_DP_INTC_PRIORITY);
    (void) CyIntSetVector(USB_1_DP_INTC_VECT_NUM, &USB_1_DP_ISR);
#endif /* (USB_1_DP_ISR_ACTIVE) */

#if (USB_1_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    USB_1_InitEpDma();
#endif /* (USB_1_EP_MANAGEMENT_DMA && CY_PSOC4) */
}


/*******************************************************************************
* Function Name: USB_1_InitComponent
****************************************************************************//**
*
...
* \globalvars
*   \ref USB_1_device
*   \ref USB_1_transferState
*   \ref USB_1_configuration
*   \ref USB_1_deviceStatus
*
*   \ref USB_1_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref USB_1_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
void USB_1_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
    ...
    {
        USB_1_hidProtocol[i] = USB_1_PROTOCOL_REPORT;
    }
    ...
    USB_1_transferState   = USB_1_TRANS_STATE_IDLE;
    USB_1_configurationChanged = 0u;
    USB_1_configuration   = 0u;
    USB_1_interfaceNumber = 0u;
    USB_1_deviceAddress   = 0u;
    USB_1_deviceStatus    = 0u;
    USB_1_lastPacketSize  = 0u;

    /* Enable component interrupts. */
#if (CY_PSOC4)
    CyIntEnable(USB_1_INTR_HI_VECT_NUM);
    CyIntEnable(USB_1_INTR_MED_VECT_NUM);
    CyIntEnable(USB_1_INTR_LO_VECT_NUM);
#else
    CyIntEnable(USB_1_BUS_RESET_VECT_NUM);
    CyIntEnable(USB_1_EP_0_VECT_NUM);

    #if (USB_1_SOF_ISR_ACTIVE)
        CyIntEnable(USB_1_SOF_VECT_NUM);
    #endif /* (USB_1_SOF_ISR_ACTIVE) */

    #if (USB_1_EP1_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_1_VECT_NUM);
    #endif /* (USB_1_EP1_ISR_ACTIVE) */

    #if (USB_1_EP2_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_2_VECT_NUM);
    #endif /* (USB_1_EP5_ISR_ACTIVE) */

    #if (USB_1_EP3_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_3_VECT_NUM);
    #endif /* (USB_1_EP5_ISR_ACTIVE) */

    #if (USB_1_EP4_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_4_VECT_NUM);
    #endif /* (USB_1_EP5_ISR_ACTIVE) */

    #if (USB_1_EP5_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_5_VECT_NUM);
    #endif /* (USB_1_EP5_ISR_ACTIVE) */

    #if (USB_1_EP6_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_6_VECT_NUM);
    #endif /* USB_1_EP6_ISR_REMOVE */

    #if (USB_1_EP7_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_7_VECT_NUM);
    #endif /* (USB_1_EP7_ISR_ACTIVE) */

    #if (USB_1_EP8_ISR_ACTIVE)
        CyIntEnable(USB_1_EP_8_VECT_NUM);
    #endif /* (USB_1_EP8_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */
    ...
    USB_1_ARB_INT_EN_REG = USB_1_DEFAULT_ARB_INT_EN;
        ...
        CyIntEnable(USB_1_ARB_VECT_NUM);
    ...
    USB_1_ARB_CFG_REG = USB_1_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, USB_1_DMA_AUTO_INTR_PRIO);
        #endif /* (USB_1_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
            ...
            CyIntEnable(CYDMA_INTR_NUMBER);
                ...
                USB_1_EP_DMA_Done_isr_StartEx(&USB_1_EP_DMA_DONE_ISR);
                USB_1_EP17_DMA_Done_SR_InterruptEnable();
                USB_1_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* (USB_1_EP_DMA_AUTO_OPT == 0u) */
        #endif /* (CY_PSOC4) */
    #endif /* (USB_1_EP_MANAGEMENT_DMA_AUTO) */
#endif /* (USB_1_EP_MANAGEMENT_DMA) */
...
/*******************************************************************************
* Function Name: USB_1_ReInitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   USB_1_device - Contains the device number of the desired Device
...
*   USB_1_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
*   \ref USB_1_deviceStatus
*
*   \ref USB_1_configurationChanged
*
*   USB_1_intiVar -  This variable is set to zero
*
*******************************************************************************/
void USB_1_Stop(void) 
{
    ...
    /* Disable D+ pull-up. */
    USB_1_USBIO_CR1_REG &= (uint8) ~ USB_1_USBIO_CR1_USBPUEN;

#if (CY_PSOC4)
    /* Disable USBFS block. */
    USB_1_POWER_CTRL_REG &= (uint32) ~USB_1_POWER_CTRL_ENABLE;
#else
...
#if (CY_PSOC4)
    CyIntDisable(USB_1_INTR_HI_VECT_NUM);
    CyIntDisable(USB_1_INTR_LO_VECT_NUM);
    CyIntDisable(USB_1_INTR_MED_VECT_NUM);
#else

    CyIntDisable(USB_1_BUS_RESET_VECT_NUM);
    CyIntDisable(USB_1_EP_0_VECT_NUM);

    #if (USB_1_SOF_ISR_ACTIVE)
        CyIntDisable(USB_1_SOF_VECT_NUM);
    #endif /* (USB_1_SOF_ISR_ACTIVE) */

    #if (USB_1_EP1_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_1_VECT_NUM);
    #endif /* (USB_1_EP1_ISR_ACTIVE) */

    #if (USB_1_EP2_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_2_VECT_NUM);
    #endif /* (USB_1_EP2_ISR_ACTIVE) */

    #if (USB_1_EP3_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_3_VECT_NUM);
    #endif /* (USB_1_EP3_ISR_ACTIVE) */

    #if (USB_1_EP4_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_4_VECT_NUM);
    #endif /* (USB_1_EP4_ISR_ACTIVE) */

    #if (USB_1_EP5_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_5_VECT_NUM);
    #endif /* (USB_1_EP5_ISR_ACTIVE) */

    #if (USB_1_EP6_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_6_VECT_NUM);
    #endif /* USB_1_EP6_ISR_REMOVE */

    #if (USB_1_EP7_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_7_VECT_NUM);
    #endif /* (USB_1_EP7_ISR_ACTIVE) */

    #if (USB_1_EP8_ISR_ACTIVE)
        CyIntDisable(USB_1_EP_8_VECT_NUM);
    #endif /* (USB_1_EP8_ISR_ACTIVE) */
...
/*******************************************************************************
* Function Name: USB_1_CheckActivity
****************************************************************************//**
*
...
{
    uint8 cr1Reg = USB_1_CR1_REG;

    /* Clear bus activity. */
    USB_1_CR1_REG = (cr1Reg & (uint8) ~USB_1_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & USB_1_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}


/*******************************************************************************
* Function Name: USB_1_GetConfiguration
****************************************************************************//**
*
...
{
    return (USB_1_configuration);
}


/*******************************************************************************
* Function Name: USB_1_IsConfigurationChanged
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USB_1_configurationChanged - This variable is set to 1 after
...
uint8 USB_1_IsConfigurationChanged(void) 
{
    ...
    if (USB_1_configurationChanged != 0u)
    {
        res = USB_1_configurationChanged;
        USB_1_configurationChanged = 0u;
    }
...
/*******************************************************************************
* Function Name: USB_1_GetInterfaceSetting
****************************************************************************//**
*
...
{
    return (USB_1_interfaceSetting[interfaceNumber]);
}


/*******************************************************************************
* Function Name: USB_1_GetEPState
****************************************************************************//**
*
...
{
    return (USB_1_EP[epNumber].apiEpState);
}


/*******************************************************************************
* Function Name: USB_1_GetEPCount
****************************************************************************//**
*
...
uint16 USB_1_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > USB_1_EP0) && (epNumber < USB_1_MAX_EP))
    {
        ...
        cntr  = ((uint16) USB_1_SIE_EP_BASE.sieEp[epNumber].epCnt0) & USB_1_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) USB_1_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= USB_1_EPX_CNTX_CRC_COUNT;
    }
    ...
    *******************************************************************************/
    static void USB_1_InitEpDma(void)
    {
        ...
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep1_dma_CHANNEL] = USB_1_ep1_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA1_ACTIVE) */

    #if (USB_1_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep2_dma_CHANNEL] = USB_1_ep2_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA2_ACTIVE) */

    #if (USB_1_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep3_dma_CHANNEL] = USB_1_ep3_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA3_ACTIVE) */

    #if (USB_1_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep4_dma_CHANNEL] = USB_1_ep4_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA4_ACTIVE) */

    #if (USB_1_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep5_dma_CHANNEL] = USB_1_ep5_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA5_ACTIVE) */

    #if (USB_1_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep6_dma_CHANNEL] = USB_1_ep6_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA6_ACTIVE) */

    #if (USB_1_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep7_dma_CHANNEL] = USB_1_ep7_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA7_ACTIVE) */

    #if (USB_1_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USB_1_ep8_dma_CHANNEL] = USB_1_ep8_dma_CHANNEL_CFG;
    #endif /* (USB_1_DMA8_ACTIVE) */

    #if (USB_1_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (USB_1_DMA1_ACTIVE)
            (void) USB_1_ep1_dma_SetInterruptCallback(&USB_1_EP1_DMA_DONE_ISR);
        #endif /* (USB_1_DMA1_ACTIVE) */

        #if (USB_1_DMA2_ACTIVE)
            (void) USB_1_ep2_dma_SetInterruptCallback(&USB_1_EP2_DMA_DONE_ISR);
        #endif /* (USB_1_DMA2_ACTIVE) */

        #if (USB_1_DMA3_ACTIVE)
            (void) USB_1_ep3_dma_SetInterruptCallback(&USB_1_EP3_DMA_DONE_ISR);
        #endif /* (USB_1_DMA3_ACTIVE) */

        #if (USB_1_DMA4_ACTIVE)
            (void) USB_1_ep4_dma_SetInterruptCallback(&USB_1_EP4_DMA_DONE_ISR);
        #endif /* (USB_1_DMA4_ACTIVE) */

        #if (USB_1_DMA5_ACTIVE)
            (void) USB_1_ep5_dma_SetInterruptCallback(&USB_1_EP5_DMA_DONE_ISR);
        #endif /* (USB_1_DMA5_ACTIVE) */

        #if (USB_1_DMA6_ACTIVE)
            (void) USB_1_ep6_dma_SetInterruptCallback(&USB_1_EP6_DMA_DONE_ISR);
        #endif /* (USB_1_DMA6_ACTIVE) */

        #if (USB_1_DMA7_ACTIVE)
            (void) USB_1_ep7_dma_SetInterruptCallback(&USB_1_EP7_DMA_DONE_ISR);
        #endif /* (USB_1_DMA7_ACTIVE) */

        #if (USB_1_DMA8_ACTIVE)
            (void) USB_1_ep8_dma_SetInterruptCallback(&USB_1_EP8_DMA_DONE_ISR);
        #endif /* (USB_1_DMA8_ACTIVE) */
    #endif /* (USB_1_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    /***************************************************************************
    * Function Name: USB_1_InitEP_DMA
    ************************************************************************//**
    *
    *  This function allocates and initializes a DMA channel to be used by the
    *  USB_1_LoadInEP() or USB_1_ReadOutEP() APIs for data
    ...
    ***************************************************************************/
                                                                    ...
                                                                    
    {
    ...
    #if (USB_1_DMA1_ACTIVE)
        case USB_1_EP1:
            USB_1_DmaChan[epNumber] = USB_1_ep1_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                    USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA1_ACTIVE) */

    #if (USB_1_DMA2_ACTIVE)
        case USB_1_EP2:
            USB_1_DmaChan[epNumber] = USB_1_ep2_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                    USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA2_ACTIVE) */
    ...
    #if (USB_1_DMA3_ACTIVE)
            case USB_1_EP3:
                USB_1_DmaChan[epNumber] = USB_1_ep3_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                        USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA3_ACTIVE) */
    ...
    #if (USB_1_DMA4_ACTIVE)
            case USB_1_EP4:
                USB_1_DmaChan[epNumber] = USB_1_ep4_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                        USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA4_ACTIVE) */
    ...
    #if (USB_1_DMA5_ACTIVE)
            case USB_1_EP5:
                USB_1_DmaChan[epNumber] = USB_1_ep5_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                        USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA5_ACTIVE) */
    ...
    #if (USB_1_DMA6_ACTIVE)
        case USB_1_EP6:
            USB_1_DmaChan[epNumber] = USB_1_ep6_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                    USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA6_ACTIVE) */
    ...
    #if (USB_1_DMA7_ACTIVE)
        case USB_1_EP7:
                USB_1_DmaChan[epNumber] = USB_1_ep7_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                        USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA7_ACTIVE) */
    ...
    #if (USB_1_DMA8_ACTIVE)
        case USB_1_EP8:
                USB_1_DmaChan[epNumber] = USB_1_ep8_DmaInitialize(USB_1_DMA_BYTES_PER_BURST,
                                                                                        USB_1_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USB_1_DMA8_ACTIVE) */
        ...
        if ((epNumber > USB_1_EP0) && (epNumber < USB_1_MAX_EP))
        {
            USB_1_DmaTd[epNumber] = CyDmaTdAllocate();

        #if (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u))
            USB_1_DmaNextTd[epNumber] = CyDmaTdAllocate();
        #endif /* (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u)) */
        }
    }
#endif /* (CY_PSOC4) */
    ...
    *  to trigger DMA re-configuration when USB_1_LoadInEP() or 
    *  USB_1_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void USB_1_Stop_DMA(uint8 epNumber) 
    {
        ...
        i = (epNumber < USB_1_MAX_EP) ? epNumber : USB_1_EP1;

        do
        {
        #if (CY_PSOC4)
            if (0u != USB_1_DmaChan[i])
            {
                USB_1_CyDmaChDisable(USB_1_DmaChan[i]);
            }
        #else
            if(USB_1_DmaTd[i] != DMA_INVALID_TD)
            {
                (void) CyDmaChDisable(USB_1_DmaChan[i]);
                CyDmaTdFree(USB_1_DmaTd[i]);
                USB_1_DmaTd[i] = DMA_INVALID_TD;
            }

            #if (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u))
                if(USB_1_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(USB_1_DmaNextTd[i]);
                    USB_1_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u)) */
        ...
        while ((i < USB_1_MAX_EP) && (epNumber == USB_1_MAX_EP));
    }
#endif /* (USB_1_EP_MANAGEMENT_DMA) */
...
#if (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u))
    /***************************************************************************
    * Function Name: USB_1_LoadNextInEP
    ************************************************************************//**
    *
    * Summary:
    ...
    ***************************************************************************/
    void USB_1_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if (mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_1_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USB_1_inLength[epNumber] - USB_1_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_1_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)USB_1_inDataPointer[epNumber] +
                                            USB_1_DMA_BYTES_PER_BURST));
            USB_1_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_1_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USB_1_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_1_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)USB_1_inDataPointer[epNumber] +
                                   (USB_1_DMA_BYTES_PER_BURST - USB_1_DMA_BYTES_REPEAT)));
        }

        /* CyDmaChSetInitialTd API is optimized to initialize TD. */
        CY_DMA_CH_STRUCT_PTR[USB_1_DmaChan[epNumber]].basic_status[1u] = USB_1_DmaTd[epNumber];
    }
#endif /* (USB_1_EP_MANAGEMENT_DMA_AUTO && (USB_1_EP_DMA_AUTO_OPT == 0u)) */
#endif /* (CY_PSOC3 || CY_PSOC5) */
                                                                        ...
                                                                        
{
    if ((epNumber > USB_1_EP0) && (epNumber < USB_1_MAX_EP))
    {
        ...
        if (length > (USB_1_EPX_DATA_BUF_MAX - USB_1_EP[epNumber].buffOffset))
        {
            length = USB_1_EPX_DATA_BUF_MAX - USB_1_EP[epNumber].buffOffset;
        }
    #endif /* (!USB_1_EP_MANAGEMENT_DMA_AUTO) */

        /* Set count and data toggle. */
        USB_1_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint8) HI8(length) | USB_1_EP[epNumber].epToggle;
        USB_1_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint8) LO8(length);
        ...
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }

        /* IN endpoint buffer is full - read to be read. */
        USB_1_EP[epNumber].apiEpState = USB_1_NO_EVENT_PENDING;

        /* Arm IN endpoint. */
        USB_1_SIE_EP_BASE.sieEp[epNumber].epCr0 = USB_1_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == USB_1_DmaTd[epNumber])
            {
                USB_1_InitEP_DMA(epNumber, pData);
            }
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) USB_1_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                USB_1_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) pData);
                USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR0, USB_1_DMA_COMMON_CFG | length |
                ...
                (void) CyDmaChDisable(USB_1_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(USB_1_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USB_1_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr));

                /* Enable DMA channel. */
                (void) CyDmaChSetInitialTd(USB_1_DmaChan[epNumber], USB_1_DmaTd[epNumber]);
                (void) CyDmaChEnable(USB_1_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */

                /* Generate DMA request. */
                USB_1_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint8)  USB_1_ARB_EPX_CFG_DMA_REQ;
                USB_1_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint8) ~USB_1_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                USB_1_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr);
                USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR1, (void*) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR0, USB_1_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR1, USB_1_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                USB_1_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(USB_1_DmaChan[epNumber]);
                ...
                #if (USB_1_EP_DMA_AUTO_OPT == 0u)
                    USB_1_inLength[epNumber] = length;
                    USB_1_inDataPointer[epNumber] = pData;

                    /* Configure DMA to send data only for first burst */
                    (void) CyDmaTdSetConfiguration(USB_1_DmaTd[epNumber],
                        (length > USB_1_DMA_BYTES_PER_BURST) ? USB_1_DMA_BYTES_PER_BURST : length,
                        USB_1_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USB_1_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    (void) CyDmaTdSetConfiguration(USB_1_DmaNextTd[epNumber], 1u,
                                                   USB_1_DmaNextTd[epNumber],
                                                   USB_1_epX_TD_TERMOUT_EN[epNumber]);

                    /* Configure DmaNextTd to clear Data Ready status. */
                    (void) CyDmaTdSetAddress(USB_1_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
                    /* Configure DMA to send all data. */
                    (void) CyDmaTdSetConfiguration(USB_1_DmaTd[epNumber], length,
                                                   USB_1_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USB_1_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr));
                #endif /* (USB_1_EP_DMA_AUTO_OPT == 0u) */

                /* Clear any potential pending DMA requests before starting DMA channel to transfer data. */
                (void) CyDmaClearPendingDrq(USB_1_DmaChan[epNumber]);
                /* Enable DMA. */
                (void) CyDmaChSetInitialTd(USB_1_DmaChan[epNumber], USB_1_DmaTd[epNumber]);
                (void) CyDmaChEnable(USB_1_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    USB_1_DmaEpBurstCnt[epNumber] = (uint8) (length / USB_1_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    USB_1_DmaEpLastBurstEl[epNumber] = (uint8) (length % USB_1_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    USB_1_DmaEpBurstCnt[epNumber] += (0u != USB_1_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    USB_1_DmaEpLastBurstEl[epNumber] = (0u != USB_1_DmaEpLastBurstEl[epNumber]) ?
                                                                          (USB_1_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (USB_1_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USB_1_DmaEpBurstCnt[epNumber]) ? USB_1_DmaEpLastBurstEl[epNumber] : (USB_1_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == USB_1_DmaEpBurstCnt[epNumber]) ? USB_1_DmaEpLastBurstEl[epNumber] : (USB_1_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    USB_1_DmaEpLastBurstEl[epNumber] |= (0u != (USB_1_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            USB_1_DMA_DESCR0_MASK : USB_1_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    USB_1_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) ((uint32) USB_1_DmaEpBufferAddrBackup[epNumber]));
                    USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR1, (void*) ((uint32) USB_1_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   USB_1_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    USB_1_CyDmaSetNumDataElements(channelNum, USB_1_DMA_DESCR0, lengthDescr0);
                    USB_1_CyDmaSetNumDataElements(channelNum, USB_1_DMA_DESCR1, lengthDescr1);
                    ...
                    if (USB_1_DmaEpBurstCnt[epNumber] > 1u)
                    {
                    ...
                    USB_1_DmaEpBurstCnt[epNumber] = USB_1_DMA_GET_BURST_CNT(USB_1_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    USB_1_CyDmaChEnable(channelNum);

                #elif (USB_1_EP_DMA_AUTO_OPT == 0u)
                    USB_1_inLength[epNumber]  = length;
                    USB_1_inBufFull[epNumber] = 0u;

                    (void) CyDmaChDisable(USB_1_DmaChan[epNumber]);
                    /* Configure DMA to send data only for first burst. */
                    (void) CyDmaTdSetConfiguration(
                        USB_1_DmaTd[epNumber], (length > USB_1_DMA_BYTES_PER_BURST) ?
                        USB_1_DMA_BYTES_PER_BURST : length,
                        USB_1_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(USB_1_DmaTd[epNumber], LO16((uint32)  USB_1_inDataPointer[epNumber]),
                                                                               LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(USB_1_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (USB_1_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    USB_1_ARB_EP_BASE.arbEp[epNumber].epCfg |= USB_1_ARB_EPX_CFG_IN_DATA_RDY;
                #endif  /* (USB_1_MANUAL_IN_EP_ARM) */
...
*   transfer after calling the USB_1_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > USB_1_EP0) && (epNumber < USB_1_MAX_EP))
    {
    #if (!USB_1_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > USB_1_GetEPCount(epNumber)) ? USB_1_GetEPCount(epNumber) : length;
    #endif /* (!USB_1_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (USB_1_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR0, USB_1_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(USB_1_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USB_1_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USB_1_DmaTd[epNumber], LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            USB_1_DmaEpLastBurstEl[epNumber] |= (USB_1_DmaEpBurstCnt[epNumber] > 2u) ? USB_1_DMA_DESCR_REVERT : 0u;
            ...
            USB_1_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            USB_1_DmaEpBurstCntBackup[epNumber]   = USB_1_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR1, (void*) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr);

            USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) ((uint32) pData));
            USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR1, (void*) ((uint32) pData + USB_1_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR0, USB_1_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR1, USB_1_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (USB_1_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                USB_1_CyDmaValidateDescriptor(channelNum, USB_1_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(USB_1_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USB_1_DmaTd[epNumber], length,  USB_1_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USB_1_DmaTd[epNumber], LO16((uint32) &USB_1_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
/*******************************************************************************
* Function Name: USB_1_LoadInEP16
****************************************************************************//**
*
*  This function performs different functionality depending on the Component’s
*  configured Endpoint Buffer Management. This parameter is defined in
...
void USB_1_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > USB_1_EP0) && (epNumber < USB_1_MAX_EP))
    {
        ...
        /* Set count and data toggle. */
        USB_1_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | USB_1_EP[epNumber].epToggle;
        USB_1_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);
        ...
        if (NULL != pData)
        {
            ...
            {
                USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        USB_1_SIE_EP_BASE.sieEp[epNumber].epCr0 = USB_1_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) pData);
                USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                ...
                /* Generate DMA request. */
                USB_1_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  USB_1_ARB_EPX_CFG_DMA_REQ;
                USB_1_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~USB_1_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR1, (void*) &USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    USB_1_DmaEpLastBurstEl[epNumber] = (0u != USB_1_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((USB_1_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (USB_1_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USB_1_DmaEpBurstCnt[epNumber]) ? USB_1_DmaEpLastBurstEl[epNumber] : (USB_1_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == USB_1_DmaEpBurstCnt[epNumber]) ? USB_1_DmaEpLastBurstEl[epNumber] : (USB_1_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: USB_1_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 USB_1_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (USB_1_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (USB_1_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USB_1_CyDmaSetDstAddress(channelNum, USB_1_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USB_1_CyDmaSetConfiguration(channelNum, USB_1_DMA_DESCR0, USB_1_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (USB_1_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            USB_1_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / USB_1_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            USB_1_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % USB_1_DMA_BYTES_PER_BURST);
            ...
            USB_1_DmaEpLastBurstEl[epNumber] |= USB_1_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR0, (void*) &USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USB_1_CyDmaSetSrcAddress(channelNum, USB_1_DMA_DESCR1, (void*) &USB_1_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
/*******************************************************************************
* Function Name: USB_1_EnableOutEP
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USB_1_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: USB_1_DisableOutEP
****************************************************************************//**
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    USB_1_FORCE_J   | Force a J State onto the D+/D– lines
*    USB_1_FORCE_K   | Force a K State onto the D+/D– lines
*    USB_1_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    USB_1_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
void USB_1_Force(uint8 bState) 
{
    ...
    */
    USB_1_USBIO_CR0_REG = bState;
}


/*******************************************************************************
* Function Name: USB_1_GetEPAckState
****************************************************************************//**
*
...
uint8 USB_1_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > USB_1_EP0) && (epNumber < USB_1_MAX_EP))
    {
        cr = USB_1_SIE_EP_BASE.sieEp[epNumber].epCr0 & USB_1_MODE_ACKD;
    }
...
*  --------------------------------------------|---------------------------
*  USB_1_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  USB_1_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref USB_1_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
{
    if (powerStatus != USB_1_DEVICE_STATUS_BUS_POWERED)
    {
    ...
    /***************************************************************************
    * Function Name: USB_1_VBusPresent
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (USB_1_VBUS_STATUS_REG & USB_1_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
#endif /* (USB_1_VBUS_MONITORING_ENABLE) */


/*******************************************************************************
* Function Name: USB_1_RWUEnabled
****************************************************************************//**
*
...
*
* \globalvars
*  USB_1_deviceStatus - checked to determine remote status
*
*******************************************************************************/
uint8 USB_1_RWUEnabled(void) 
{
    uint8 result = USB_1_FALSE;

    if (0u != (USB_1_deviceStatus & USB_1_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = USB_1_TRUE;
    }
...
/*******************************************************************************
* Function Name: USB_1_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(USB_1_CR0_REG & USB_1_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: USB_1_EnableSofInt
****************************************************************************//**
*
...
void USB_1_EnableSofInt(void) 
{
    ...
    USB_1_INTR_SIE_MASK_REG |= (uint32) USB_1_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: USB_1_DisableSofInt
****************************************************************************//**
*
...
void USB_1_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: USB_1_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   USB_1_BCD_PORT_SDP     | Standard downstream port detected
    *   USB_1_BCD_PORT_CDP     | Charging downstream port detected
    *   USB_1_BCD_PORT_DCP     | Dedicated charging port detected
    *   USB_1_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   USB_1_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 USB_1_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = USB_1_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = USB_1_POWER_CTRL_REG;
        USB_1_POWER_CTRL_REG = USB_1_DEFAULT_POWER_CTRL_VBUS\
                            & (~USB_1_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~USB_1_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        USB_1_POWER_CTRL_REG |= USB_1_DEFAULT_POWER_CTRL_PHY\
                                         | USB_1_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (USB_1_NO_DCD)
        CyDelay(USB_1_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = USB_1_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            USB_1_POWER_CTRL_REG |= USB_1_POWER_CTRL_ENABLE_DM_PULLDOWN;
            USB_1_CHGDET_CTRL_REG |= USB_1_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (USB_1_USBIO_CR1_REG & USB_1_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (USB_1_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        USB_1_CHGDET_CTRL_REG = USB_1_CHGDET_CTRL_PRIMARY;
        CyDelay(USB_1_BCD_PRIMARY_WAIT);
        cr1RegVal = USB_1_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (USB_1_CHGDET_CTRL_REG & USB_1_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & USB_1_USBIO_CR1_DM0))
            {
                result = USB_1_BCD_PORT_SDP;
            }
            else
            {
        ...
        {
            USB_1_CHGDET_CTRL_REG = USB_1_CHGDET_CTRL_DEFAULT \
                                                | USB_1_CHGDET_CTRL_SECONDARY;
            CyDelay(USB_1_BCD_SECONDARY_WAIT);
            cr1RegVal = USB_1_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & USB_1_USBIO_CR1_DP0))
                {
                    result = USB_1_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                else
                {
                    ...
                    */
                    result = USB_1_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        USB_1_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        USB_1_POWER_CTRL_REG = bkPwrCtrl;
        USB_1_USBIO_CR1_REG |= (uint32) USB_1_USBIO_CR1_IOMODE;
        USB_1_USB_CLK_EN_REG = ~USB_1_USB_CLK_CSR_CLK_EN;
...
#if (USB_1_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: USB_1_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USB_1_LPM_STAT_REG & USB_1_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: USB_1_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USB_1_LPM_STAT_REG & USB_1_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: USB_1_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - USB_1_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - USB_1_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - USB_1_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void USB_1_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = USB_1_LPM_CTRL_REG & (uint32) ~USB_1_LPM_CTRL_ACK_NYET_MASK;

        USB_1_LPM_CTRL_REG = lpmCtrl | ((uint32) response & USB_1_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: USB_1_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - USB_1_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - USB_1_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - USB_1_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 USB_1_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) USB_1_LPM_CTRL_REG & (uint32)USB_1_LPM_CTRL_ACK_NYET_MASK);
    }
